...
public class Scheduling {
    ...
    try {   
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (line.startsWith("process")) {
          ...
          sum += ticketsnum;
          processVector.addElement(new sProcess(cputime, ioblocking, 0, 0, 0, ticketsnum));
        }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      out.println("Process #\tTickets\tCPU Time\tIO Blocking\tCPU Completed\tCPU Blocked");
      for (i = 0; i < processVector.size(); i++) {
        sProcess process = (sProcess) processVector.elementAt(i);
        out.print(Integer.toString(i));
        if (i < 100) { out.print("\t\t"); } else { out.print("\t"); }
        out.print(Integer.toString(process.ticketsnum));
        if (process.ticketsnum < 100) { out.print(" \t\t"); } else { out.print(" \t"); }
